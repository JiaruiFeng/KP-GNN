...
from layers.feature_encoder import FeatureSumEncoder,FeatureConcatEncoder
import math
...
class GNN(nn.Module):
    ...
    """
    def __init__(self,num_layer,gnn_layer,init_emb,num_hop1_edge,max_edge_count,max_hop_num,max_distance_count,JK="last",norm_type="batch",virtual_node=True,
                 residual=False,use_rd=False,wo_peripheral_edge=False,wo_peripheral_configuration=False,drop_prob=0.1):
        ...
        if not wo_peripheral_configuration:
            ...
            self.peripheral_configuration_embedding=FeatureConcatEncoder(configuration_feature_dims,self.output_dk,padding=0)
    ...
    def forward(self,data):
        ...
        if (not self.wo_peripheral_configuration) and peripheral_configuration_attr is not None:
            peripheral_attr+=torch.sigmoid(self.pcw)*self.peripheral_configuration_embedding(peripheral_configuration_attr) #N * K * H
        if self.virtual_node:
...
class GNNPlus(nn.Module):
    ...
    def __init__(self,num_layer,gnn_layer,init_emb,num_hop1_edge,max_edge_count,max_hop_num,max_distance_count,JK="last",norm_type="batch",virtual_node=True,
        ...
        if not wo_peripheral_configuration:
            configuration_feature_dims=[max_distance_count+1 for _ in range(max_hop_num+1)]
            self.peripheral_configuration_embedding=FeatureConcatEncoder(configuration_feature_dims,self.hidden_size,padding=0)
            self.pcw=nn.Parameter(torch.rand(1),requires_grad=True)
    ...
    def forward(self,data):
        ...
        if (not self.wo_peripheral_configuration) and peripheral_configuration_attr is not None:
            peripheral_attr+=torch.tanh(self.pcw)*self.peripheral_configuration_embedding(peripheral_configuration_attr) #N * K * H
