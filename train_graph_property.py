...
def main():
    ...
    parser.add_argument('--task',type=int,default=0,choices=(0,1,2),help='number of task')
    ...
    train_dataset = GraphPropertyDataset(path, split='train', pre_transform=T.Compose([pre_transform,rd_feature]),transform=transform)
    val_dataset = GraphPropertyDataset(path, split='val', pre_transform=T.Compose([pre_transform,rd_feature]),transform=transform)
    test_dataset = GraphPropertyDataset(path, split='test', pre_transform=T.Compose([pre_transform,rd_feature]),transform=transform)
